...
import { O1_data_to_collect_html_selector } from "./lib/selectors";
import { I1_data_to_collect_html_selector, I2_collected_full_html_element } from "./lib/types";
...
export const f1_collect_fb_raw_html_elements_obj = async (page: Page): Promise<Record<string, any>> => {
    ...
    await Promise.all(
        Object.values(O1_data_to_collect_html_selector).map((selector) =>
            page.waitForSelector(selector as string, { timeout: 15000 }).catch(() => null)
        )
    );
